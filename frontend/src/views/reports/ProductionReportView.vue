...
import { downloadReportCsv, getProductionReport } from '@/services/reports'
import { useProvidersStore } from '@/stores/providers'
import { toLocalDateString } from '@/lib/date'
import type { ProductionReport } from '@/types/reports'

/** Production Report (design doc §4.1) — admin only, financial. */
const { t } = useI18n()
...
async function fetchReport() {
  ...
  error.value = false
  try {
    report.value = await getProductionReport({
  ...
  <div class="flex flex-col gap-4">
    ...
    >
      <template #extra-filters>
        <div class="flex flex-col gap-1">
          <label for="production-dentist" class="text-sm text-surface-600 dark:text-surface-400">
            {{ t('reports.filters.dentist') }}
          </label>
          <Select
            id="production-dentist"
            v-model="dentistId"
    ...
    <div v-if="report" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div class="rounded-lg border border-surface-200 p-4 dark:border-surface-700">
        <p class="text-sm text-surface-500">{{ t('reports.production.total') }}</p>
        <p
          dir="ltr"
    ...
    <DataTable
      v-if="report"
      :value="report.summary.by_dentist"
      class="mb-2"
      data-testid="production-by-dentist"
    >
      <template #header>{{ t('reports.production.byDentist') }}</template>
      <Column field="dentist" :header="t('reports.production.columns.dentist')" />
      <Column :header="t('reports.production.columns.amount')">
        <template #body="{ data }"
    ...
    >
      ...
      <Column field="invoice_number" :header="t('reports.production.columns.invoiceNumber')" />
      <Column field="patient" :header="t('reports.production.columns.patient')" />
      <Column :header="t('reports.production.columns.date')">
        <template #body="{ data }"
          ><span dir="ltr">{{ data.date }}</span></template
        >
      </Column>
      <Column field="description" :header="t('reports.production.columns.description')" />
      <Column field="dentist" :header="t('reports.production.columns.dentist')" />
      <Column :header="t('reports.production.columns.amount')">
        <template #body="{ data }"
          ><span dir="ltr">{{ data.amount }}</span></template

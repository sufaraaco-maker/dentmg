...
|---|---|---|---|---|---|
| 1 | **Phase 2.6b: Timeline UI** | 2.6a (Foundation — events/listener/security/endpoint) implemented and opened as a PR; once merged, 2.6b builds `ActivityTimeline.vue`, the `timeline` tab, Billing's Payment History wiring, Overview preview, i18n, per the design doc's approved 2-PR split (§16 decision 7) | High — the roadmap's own next priority, and the last sub-phase of Phase 2 | Medium (UI only — the higher-risk foundation work is 2.6a) | 2.6a's PR merged first |
...
tests, 9 controller/security tests), Pint clean. **2.6b (Timeline UI) not yet started**, per the
design doc's approved 2-PR split (§16 decision 7).

...
1510/1510/1510 across en/ar/tr. Committed as a single commit on `feature/english-default-branding-and-image-uploads`,
**PR #48 opened against `main`, not yet merged — held for explicit user approval** after CI. The
...
`docs/decisions.md`'s 2026-08-13 entry for full detail. |
| **Repo HEAD at time of writing** | `main` at merge commit `173ee2a` (PR #45 — docs close-out for Phase 5C/PR #43/PR #44, merged after resolving a real content conflict with PR #46 in `CHANGELOG.md`/`docs/PROJECT_STATUS.md`, both of which had queued edits to the same "Unreleased"/header sections; following PR #46's `notifications.spec.ts` CI-retry self-collision fix, following PR #44's E2E notification-lookup disambiguation fix, following PR #43's `appointment.created` notification fix, following PR #41's Phase 5C). Post-merge CI on `main` (run `31617477352`) showed **1 failure, unrelated to either merge** — `role-permissions.spec.ts`'s "admin toggles a permission off..." test (Phase 4, untouched by PR #45/#46's purely-docs diff), failing identically on Playwright's own retry; isolated before dismissing it (confirmed PR #45's merge changed only `CHANGELOG.md`/`TECH_DEBT.md`/`docs/PROJECT_STATUS.md`, and this exact test had passed cleanly in both of the same day's two prior full runs with identical code) then re-ran `workflow_dispatch` on `main` (run `31618315687`) to confirm rather than assume — **fully green, Backend/Frontend/E2E all `success`, 61/61 E2E passed, no flakes**, confirming the one failure was transient CI-runner flakiness, not a regression from either PR. |

...
| **Repo HEAD at time of writing** | `main` at `14416e9` (PR #36 merge — Phase 3 docs closeout); Phase 4 Steps 1-5 implemented on `feature/phase4-permissions-foundation`, pushed and CI triggered, not yet merged (no PR opened) — see `docs/modules/phase4-permissions-audit-design.md`. |
...
| Roles & Permissions | **Extended by Phase 4** (2026-08-09, all 5 steps on `feature/phase4-permissions-foundation`, pushed/CI-triggered, not yet merged) | ✅ `UserRole` backed enum + new `permissions`/`role_permissions` catalog-and-matrix, all 27 Policies on `hasPermission()`, full `audit_logs` overhaul (old/new diffs, IP/UA, auth events, immutability) | ✅ `PermissionsView.vue` (matrix, self-lockout-aware, responsive) + `AuditLogsView.vue` (general viewer, filters, diff expansion) — admin-only Sidebar entries/routes | ✅ 1145/1145 backend, 969/969 frontend, `role-permissions.spec.ts`/`audit-log.spec.ts`/`permissions.spec.ts` E2E (every scenario passing locally at least once; CI is the authoritative run) | `docs/modules/roles-permissions.md` (superseded in substance by `docs/modules/phase4-permissions-audit-design.md`, formally marked superseded 2026-08-09) | 3 roles (admin/dentist/receptionist) unchanged — deliberately not a full role hierarchy (see `docs/decisions.md` 2026-08-09), fine-grained permissions layered on top instead. Awaiting CI + user's full-diff review before PR. |
...
**Current (2026-08-09): Phase 4 (Advanced Permissions & Audit) — all 5 steps implemented.** Permissions
Foundation, Policy refactor, Audit Overhaul, Frontend, and E2E/docs closure are all done on
`feature/phase4-permissions-foundation` (Backend 1145/1145, Frontend 969/969, full E2E coverage of the
design doc's §9 required scenarios — behavior-preservation via the untouched pre-existing
`permissions.spec.ts` tests still passing, self-lockout, matrix-edit-persists-and-audits, all 4 endpoints'
403s — every scenario passing locally at least once; see §2's 2026-08-09 rows and §4/CHANGELOG.md for full
per-step detail). Final 3-locale i18n parity re-verified after Step 5 (1453/1453/1453, zero drift).
`docs/decisions.md`'s role-hierarchy-still-deferred note and `TECH_DEBT.md`'s Appointment-audit-log item both
updated. Pushed to `origin/feature/phase4-permissions-foundation`, full CI (Backend/Frontend/E2E) triggered
via `workflow_dispatch` for the authoritative signal this environment's own documented local Docker latency
can't reliably give. **No PR opened yet** — per the user's explicit instruction, next is CI confirmation
followed by the user's own review of the complete diff before any PR.
...
current status; §2's Timeline and the paragraph above are the current source of truth.)*

...
passed, no flakes, run `31615676731`). **English default UI language, in-app branding, clinic logo + user
avatar uploads (2026-08-13)** — a direct user request, cross-cutting rather than the next roadmap phase; per
this file's own standing rule, `git log`/`gh pr list`/`gh run list` were re-verified current before starting
(nothing stale found). Default locale flipped `ar`→`en` (`frontend/src/locales/index.ts`'s
`detectInitialLocale()`, `index.html`'s static `lang`/`dir`/font-preload order, `AVAILABLE_LOCALES`
reordered) — a stored per-user preference is still always honored, only the cold-start default changed. The
existing brand mark (`frontend/public/favicon.svg`, previously only the browser-tab icon) is now shown
in-app via a new `AppLogo.vue` on the login screen, sidebar header, and mobile header. **Reopens
`docs/modules/settings-design.md`'s V1 deferral of clinic logo upload** (that deferral was explicitly
conditional on "a real consumer" existing — Practice Settings' own live upload/preview loop now is one; see
`docs/decisions.md`'s 2026-08-13 entry for the full reasoning) and adds a new, previously-undiscussed user
avatar upload to the Users add/edit dialog (edit-only — no user `id` exists yet on the create step). Both
stand up the app's first-ever use of the `public` storage disk (`storage:link` wired into both Docker
entrypoints behind the same single-owner `RUN_MIGRATIONS` guard that already protects `migrate --force`
from a 3-container race; production `docker/nginx/default.prod.conf` gained a `location /storage` block it
never needed before, since nothing served from `/storage` previously) — deliberately not an extension of
Imaging/Documents' auth-gated streaming pattern, which is wrong for anything that must render pre-login.
Two additive migrations (`clinic_settings.logo_disk`/`logo_path`, `users.avatar_disk`/`avatar_path`), 4 new
endpoints, a shared `ImagePickerField.vue` backing both upload screens. Backend: Pint clean (571 files),
**1224/1224 tests green** (1214 baseline + 10 new — 5 each in `ClinicSettingTest`/`UserTest`, zero
regressions, full local suite re-run not just the new files); local
`phpstan analyse` reproduced this environment's own already-documented mass false-positive pattern
(`TECH_DEBT.md`'s "Local `phpstan analyse` broken in this dev container" entries — confirmed by `User::$role`,
a long-established real column, being flagged identically to the new `avatar_path`/`avatar_disk`), so CI's
own PHPStan step remains the authoritative check, not yet run since this work is not yet pushed. Frontend:
`vue-tsc`/ESLint clean repo-wide; `prettier --check` clean on every touched file (the pre-existing
300+-file repo-wide "failure" outside this change is a Windows `git core.autocrlf` artifact, confirmed via
a byte-identical `--write` diff on an untouched file — the same class of local-only false positive as the
PHPStan one, unrelated to this change); 11 new/updated Vitest tests (`UsersView.test.ts`,
`locales/index.test.ts`, `PracticeSettingsView.test.ts` fixture completed); i18n parity re-verified at
1510/1510/1510 across en/ar/tr. Committed as a single commit on `feature/english-default-branding-and-image-uploads`. The
`pull_request`-triggered CI run (`31730606592`) caught one real issue neither local `vue-tsc --noEmit` nor
this file's own earlier verification pass had: `npm run build`'s stricter `vue-tsc -b` (project-reference/
build mode) failed on `UsersView.vue`'s `ImagePickerField` binding not accounting for `AuthUser.avatar_url`'s
deliberate optionality (`Type 'string | null | undefined' is not assignable to type 'string | null'`) — the
Backend job and every other Frontend step (type-check, lint, format, unit tests) had already passed on that
same run, only the separate build step failed. Fixed with `editingUser.avatar_url ?? null` at the one call
site that needed it, verified locally via `npm run build`, pushed as a follow-up commit. See
`docs/decisions.md`'s 2026-08-13 entry for full detail. **Merged via PR #48** (`b0a0b6c`, 2026-08-13,
18:51 UTC) after explicit user approval. **Post-merge CI on `main` went red** (run `31732904218`) — **9 E2E
failures across 3 unrelated classes, none of them backend/application regressions**: (1) `e2e/rtl-ltr.spec.ts`
still hardcoded the pre-PR-#48 assumption that Arabic was the default locale in 2 tests, so a plain `login()`
no longer landed on the RTL/Arabic state either test expected; (2) `e2e/notifications.spec.ts` failed 6 tests
with "No available slots" — a genuinely pre-existing, date-dependent bug: `findAvailableSlot()` always books
"tomorrow," but `DemoDataSeeder` only seeds working hours Sunday–Thursday, so any CI run landing on a
Wednesday/Thursday (tomorrow = Friday/Saturday) returns zero slots — unrelated to PR #48's locale change,
just newly visible because 2026-08-14 is a Friday; (3) 1 already-documented `role-permissions.spec.ts`
CI-runner flake (same class as PR #45's post-merge run). **Fix (1)**: rewrote `rtl-ltr.spec.ts` for the new
default (English/LTR from a plain login; new `forceArabicLocale()` fixture helper keeps RTL rendering
covered via two tests that explicitly force it) — **merged via PR #49** (`f4fa203`, 2026-08-13, 20:02 UTC);
post-merge CI confirmed the RTL regression gone, **6 remaining failures, all `notifications.spec.ts`**, the
`role-permissions.spec.ts` flake did not recur (run `31738895281`). **Fix (2)**: applied the identical
pattern `appointments.spec.ts`'s `ensureWorkingHours` already uses for this bug class — unconditionally
`POST` a wide (08:00–20:00) working-hours shift for whatever day-of-week the target date falls on before
querying availability; no production/application code touched, a test-fixture gap only — **merged via
PR #50** (`eabd267`, 2026-08-13, 22:07 UTC); **post-merge CI on `main` fully green — Backend, Frontend, and
E2E all `success`, 62/62 E2E passed, 0 failures** (run `31748627267`), closing out all 3 regression classes
from PR #48's post-merge run. |
| **Repo HEAD at time of writing** | `main` at merge commit `3fb479e` (**PR #52** — full removal of the AI Assistant module, per explicit user request following a read-only discovery audit; a final human-reviewed pass before merge found one real, non-critical leftover — see `TECH_DEBT.md`'s new "Orphaned `ai_interaction_log` permission-matrix i18n keys" entry — user approved merging as-is). Post-merge CI on `main` (run `31838479894`) is **fully green — Backend, Frontend, and the real E2E run all `success`, 60/60 E2E passed (59 passed + 1 flaky-then-passed on `notifications.spec.ts`, an unrelated pre-existing-class timing race, not a regression)**. Following **PR #50**/**PR #49**/**PR #48**'s English-default/branding chain (still fully intact and unaffected by this removal). Full account: `CHANGELOG.md`'s "Removed — AI Assistant module" entry. |
| **Milestone** | **Phase 1 — Foundation Complete** (2026-08-07) — baseline for all future development; see §2 for the verification this milestone rests on. **Phase 2.1-2.5 (Foundation/Billing/Medical History/Laboratory/Documents) all merged 2026-08-07/08 via PR #18/#20/#22/#24/#27.** **Phase 2.6 (Timeline) design-approved 2026-08-08; 2.6a (Foundation) merged 2026-08-09 via PR #31; 2.6b (UI) merged 2026-08-09 via PR #32** (E2E fix via PR #33) — see `docs/modules/patient-timeline-redesign-design.md`. **Milestone: "Phase 2 — Patient Profile Redesign Complete" (2026-08-09)**, closed out via PR #34 — all 7 sub-phases (2.1-2.6) merged to `main`, post-merge CI green including the real E2E run. **Milestone: "Phase 3 — Dashboard 2.0 Complete" (2026-08-09)**, closed out via **PR #35** (`0f04b35`) — post-merge CI on `main` fully green (Backend/Frontend/E2E) — see `docs/modules/dashboard-2.0-design.md`. **Milestone: "Phase 4 — Advanced Permissions & Audit Complete" (2026-08-09)**, closed out via **PR #37** (`0bdf3d8`) — post-merge CI on `main` fully green (Backend/Frontend/E2E, run `31333946387`) — see `docs/modules/phase4-permissions-audit-design.md`. **Milestone: "Phase 5 — Notification System Complete" (2026-08-11)**, closed out via **PR #39** (merge commit `204194f`) — a real in-app Notification Center plus the queue worker and scheduler that had never existed; pre-PR review fixed 5 further issues same day (2 marked SECURITY); a pre-merge `workflow_dispatch` E2E gate found and fixed 2 more genuinely real, pre-existing bugs before merging — post-merge CI on `main` fully green (Backend/Frontend/E2E, run `31545464355`, 56/57 E2E passed) — see `docs/modules/notifications-design.md`. **Phase 5C (scheduled/administrative notifications, types 9-13) implemented 2026-08-12** — design doc `docs/modules/notifications-phase-c-d-design.md`, full implementation account in its §19-20 — Backend 1211/1211 green; both verification gaps left open at implementation time (full frontend Vitest, real-browser/E2E) closed same day — full frontend Vitest 1003/1005 (2 pre-existing environment flakes, re-verified in isolation), real-browser/E2E closed via new CI-run `notifications.spec.ts` coverage rather than local Playwright (confirmed infeasible in this Alpine dev container). Pre-merge `workflow_dispatch` gate fully green on the second attempt (run `31584698456` — Backend/Frontend/E2E all `success`, E2E 59/59; the first attempt, run `31583701772`, caught one real CI-only Pint violation, fixed in a follow-up commit). **Milestone: "Phase 5C — Scheduled & Administrative Notifications Complete" (2026-08-12)**, closed out via **PR #41** (merge commit `1de1316`); post-merge CI on `main` fully green (Backend/Frontend/E2E, run `31585621311`); docs close-out via **PR #42**. **`appointment.created` notification (2026-08-12)** — a real gap found diagnosing a user report (the assigned dentist never got notified when a new appointment was created for them): `AppointmentService::create()` never dispatched `PatientActivityOccurred` at all, unlike every other appointment status transition, so neither the Notification System nor the Patient Timeline ever recorded appointment creation. Fixed — new `AppointmentCreatedNotification` (assigned dentist only, structurally excluding the creator and every other admin/receptionist/dentist), registered in `NotificationRules` (now 14 types); Backend 1214/1214, live end-to-end verified against the real dev stack; **merged via PR #43** (`f77d002`). Post-merge CI then caught a real, genuine regression in `notifications.spec.ts` — once one appointment could carry two notification types for the same recipient, the E2E helper `notificationIdFor()`'s subject_id-only lookup could no longer disambiguate them, failing 5 tests (1 real content-mismatch, 4 cascading from its retry). A test-assumption bug, not a backend defect — fixed by adding an optional `type` filter to the helper, no backend/actor-exclusion/feature change; **merged via PR #44** (`c4ec273`). Post-merge CI on `main` re-run and **fully green — Backend, Frontend, and E2E all `success`, 60/60 passed, no flakes** (run `31602557965`). **Notification E2E retry-fragility fix (2026-08-12)** — the module-level `slotHour` counter that caused PR #44's own retry cascade was still latently fragile after that fix; replaced with a real `GET /available-slots` query, **merged via PR #46** (`498669f`); post-merge CI on `main` fully green (Backend/Frontend/E2E, 61/61 E2E passed, no flakes, run `31615676731`). Phase 5D (email) remains deferred to its own future cycle — see §12's "Immediate next step." **English default UI language, in-app branding, clinic logo + user avatar uploads, merged via PR #48** (`b0a0b6c`, 2026-08-13) — cross-cutting, user-requested, not a roadmap phase; default locale `ar`→`en`, brand mark shown in-app for the first time, clinic-logo/user-avatar upload (the app's first use of the `public` storage disk). Post-merge CI found 9 E2E failures across 3 unrelated classes (none application regressions): `rtl-ltr.spec.ts` still assumed Arabic was the default — **fixed via PR #49** (`f4fa203`); `notifications.spec.ts` hit a pre-existing Friday/Saturday working-hours seeding gap — **fixed via PR #50** (`eabd267`); 1 already-documented `role-permissions.spec.ts` CI-runner flake that did not recur. **Post-merge CI on `main` after PR #50 fully green — Backend, Frontend, and E2E all `success`, 62/62 E2E passed, 0 failures** (run `31748627267`). See this row's own header-cell PR #48 entry above and `CHANGELOG.md` for the full account. **AI Assistant module fully removed, merged via PR #52** (`3fb479e`, 2026-08-14) — per explicit user request following a read-only discovery audit (§7 is kept as a placeholder "Removed" notice rather than renumbered away, so every other section's `§N` cross-references stay valid); a dedicated human-review pass before merge (separate from the removal work itself) verified `ClinicalNoteDetailView.vue`'s `saveDraft()` edit was surgically clean, confirmed the two new migrations are safe on both fresh and existing databases, and confirmed the 72-file diff stayed within scope — it found one real, non-critical leftover (orphaned Permissions-matrix i18n keys for the deleted `ai_interaction_log` permission group, logged in `TECH_DEBT.md`), which the user approved merging past. **Post-merge CI on `main` fully green** (Backend/Frontend/E2E, 60/60 E2E passed, 1 unrelated flaky-then-passed, run `31838479894`). Full account in `CHANGELOG.md`'s "Removed — AI Assistant module" entry and this file's §7. **Production
hardening pass (2026-08-18)**, per explicit user request to close the remaining gaps between Treatment
Plans/Payments/Billing and this project's own "Production Ready" bar, and to raise backend test rigor: (1)
closed `TECH_DEBT.md`'s "Orphaned `ai_interaction_log`" item — 6 i18n keys removed from en/ar/tr, a new,
deliberately irreversible migration purges stale `permissions`/`role_permissions` rows on any non-fresh
environment, **merged via PR #55**; (2) added a new `backend-postgres` CI job running the full backend suite
against real PostgreSQL 17, not just `phpunit.xml`'s forced SQLite — its very first real run caught a
genuine bug, `AuditLogService`'s documented fail-open guarantee silently breaking on Postgres inside a
caller's own wrapped `DB::transaction()` (`RolePermissionService::updateMatrix()`,
`TreatmentPlanService::accept()`/`cancel()`) — fixed by wrapping the audit write in its own nested
transaction (a real Postgres `SAVEPOINT`), **merged via PR #54**; a same-day follow-up hotfix (**PR #59**)
was needed after the AI-cleanup migration's `Cache::forget()` call broke the new Postgres job (no Redis
service there; fixed with `CACHE_STORE=array` for that job's plain `artisan` steps) — post-merge CI on
`main` briefly went red, then green again after the hotfix merged; (3) wrote and CI-verified permanent
Playwright E2E suites for the three modules that had shipped without one — `treatment-plans.spec.ts`
(**PR #56**), `payments.spec.ts` (**PR #57**), and `billing.spec.ts` alongside 49 new backend Feature tests
closing `InvoiceControllerTest.php`'s HTTP-layer gap and a new `docs/modules/billing.md` (**PR #58**). Each
E2E branch went through multiple real `workflow_dispatch` rounds — every failure found was a genuine
test-script bug (PrimeVue toast/dialog/status-tag text ambiguities under Playwright's strict mode, a
paginated-response-treated-as-a-bare-array crash, a missing `logout()` between actor switches within one
test, one test's UI-driven two-plan setup replaced with direct API calls after proving unreliable in this
CI environment specifically), never an application defect. Found and logged one new, previously-undocumented
gap (no UI links an appointment to a Treatment Plan item, despite full backend support) and fixed one
already-stale doc claim this same pass almost missed (`PROJECT_CONTEXT.md`'s frozen AI Assistant paragraph
never got a removal correction note, and this file's own §9 table still listed the Laboratory
`BelongsToPatient` SQL bug as open two weeks after it was actually fixed). Also confirmed, not assumed: a
repo-wide search found zero remaining functional AI Assistant code, and i18n key parity holds at
1462/1462/1462 across en/ar/tr. Full account in `CHANGELOG.md`'s newest entries and `TECH_DEBT.md`'s
RESOLVED notes. |
...
roadmap (Dashboard → AI Assistant) was merged to `main`; AI Assistant was later **fully removed** (2026-08-14,
per explicit user request — see this file's own newest entries below and `CHANGELOG.md`'s "Removed — AI
Assistant module"), leaving 16 modules. **Treatment Plans, Billing, and Payments — the three modules that
sat just below this project's "Production Ready ✅" bar for lacking a permanent E2E suite — closed that gap
via a 2026-08-18 production-hardening pass** (Billing also gained the backend HTTP-layer Feature-test
coverage and final `modules/billing.md` doc it was separately missing); see §3's module table for the
per-module detail and this file's own newest header-cell entry above for the full account. Two post-roadmap
initiatives are now running in
...
| Users | Done | ✅ CRUD, soft delete, avatar upload (added 2026-08-13, not yet merged) | ✅ `UsersView` (avatar picker in edit dialog + table thumbnail) | ✅ | `docs/modules/users.md` | Self-delete blocked; admin-only write. |
...
| Treatment Plans | **Production Ready ✅** (merged PR #1, 2026-07-25; clinic-wide list PR #12, 2026-08-02; permanent E2E suite closed the last gap, production-hardening pass PR #56, 2026-08-18) | ✅ plan/item lifecycle | ✅ per-patient tabs + clinic-wide `TreatmentPlansView` | 505/505 backend + 541/541 frontend at original merge (permission/failure-case coverage re-confirmed comprehensive, 88 tests, during the 2026-08-18 pass); `frontend/e2e/treatment-plans.spec.ts` (new) CI-confirmed green | `docs/modules/treatment-plans.md`, `-design.md` | Reuses `dental_conditions` as V1 pricing catalog — flagged as revisit-when-multi-tenant (§9). New, previously-undocumented gap found 2026-08-18: no UI links an appointment to a plan item despite full backend support (§9). |
| Billing | **Production Ready ✅** (merged PR #1, 2026-07-25; HTTP-layer Feature tests, permanent E2E suite, and the final `modules/billing.md` doc all closed via the production-hardening pass, PR #58, 2026-08-18) | ✅ Invoice lifecycle | ✅ Invoice tab/detail | 65/65 backend Unit (Service layer) + **49 new Feature tests** (`InvoiceTest`/`InvoiceItemTest`, the HTTP-layer gap `InvoiceControllerTest` never covered) — all verified against real PostgreSQL; `frontend/e2e/billing.spec.ts` (new) CI-confirmed green | `docs/modules/billing.md` (new, final doc) + `-design.md` | Previously the weakest-tested module; now at parity with every other Production Ready module. |
| Payments | **Production Ready ✅** (merged PR #1, 2026-07-25; concurrency fix PR #2, same day; permanent E2E suite closed the last gap, production-hardening pass PR #57, 2026-08-18) | ✅ record/apply/refund | ✅ Payments tab + Invoice Payments panel | 619/619 backend Unit + 22/22 Feature (strengthened 2026-08-18: refund's original-payment-row-unmutated assertion made explicit); `frontend/e2e/payments.spec.ts` (new) CI-confirmed green | `docs/modules/payments-design.md` | Refund is a new signed-negative row, never edits the original — now explicitly regression-tested at both Feature and E2E level. |
...
| Settings | **Production Ready ✅** (PR #8, **merged** 2026-07-30 — corrects stale "not merged" prose elsewhere, §0); **clinic logo upload added 2026-08-13**, not yet merged — see this file's 2026-08-13 entry above | ✅ `ClinicSetting`, `BillingSetting` API, My Account, clinic logo upload | ✅ Settings views + header avatar menu (now shows the real uploaded avatar when set) | 877/877 backend (22 module-specific) + 672/672 frontend + 32/32 E2E as of PR #8; +10 new backend Feature tests (full suite re-run 1224/1224, zero regressions) and +11 new/updated Vitest tests for the 2026-08-13 upload work | `docs/modules/settings-design.md` | Multi-branch, notification settings still deliberately out of V1; clinic logo upload's V1 deferral **reversed 2026-08-13** (see `docs/decisions.md`). |
...
| 9 | `feature/ai-assistant` | AI Assistant module | Dashboard Insights, Smart Search, Reports, gated Clinical Notes/Treatment Suggestions | **Merged** 2026-07-31 (`644fed6`); **fully removed 2026-08-14 via PR #52** (`3fb479e`) — see `CHANGELOG.md`'s "Removed — AI Assistant module" |
...
| 11 | `feature/ai-assistant-api-key` | Settings-managed Anthropic API key | Encrypted key column, `AiAssistantService::client()` prefers it over `.env` | **Merged** 2026-08-02; **fully removed 2026-08-14 via PR #52** along with the rest of AI Assistant |
...
**Rows #40 and #42 intentionally not yet backfilled here** — both are docs-only close-outs (Phase 5, Phase
5C) with no code diff of their own to describe; this table picks them up if/when that narrower gap is
closed rather than duplicating the header cell's/CHANGELOG's fuller narrative here.
...
| **#45** | `docs/appointment-created-and-e2e-fix-close-out` | Docs-only: close out Phase 5C (PR #41) + `appointment.created`/E2E disambiguation fixes (PR #43/#44) | `CHANGELOG.md`/`TECH_DEBT.md`/`docs/PROJECT_STATUS.md` entries for PR #41/#43/#44; no code changes. Real merge conflict against PR #46 in `CHANGELOG.md`/`docs/PROJECT_STATUS.md` (both queued edits to the same sections) — merged `main` into the branch and hand-resolved, ordering entries newest-first; re-ran the `workflow_dispatch` CI gate post-resolution | **Merged** 2026-08-12 (`173ee2a`); post-merge CI showed 1 unrelated, isolated flake (`role-permissions.spec.ts`, no code touched by this merge) — re-run on `main` fully green (Backend/Frontend/E2E, 61/61 E2E passed, no flakes, run `31618315687`) |
| **#48** | `feature/english-default-branding-and-image-uploads` | English default UI language, in-app branding, clinic logo + user avatar uploads | Default locale `ar`→`en` (stored per-user preference still honored); `AppLogo.vue` on login/sidebar/mobile header; clinic-logo + user-avatar upload (2 additive migrations, 4 endpoints, shared `ImagePickerField.vue`, app's first use of the `public` storage disk); pre-merge CI caught a real `vue-tsc -b` build-mode error, fixed in a follow-up commit | **Merged** 2026-08-13 (`b0a0b6c`) after explicit user approval; post-merge CI on `main` went red — 9 E2E failures across 3 unrelated classes, closed via PR #49/#50 below |
| **#49** | `fix/e2e-rtl-ltr-english-default` | Fix: `rtl-ltr.spec.ts` still assumed Arabic was the default locale after PR #48 | Rewrote the spec for the new default (`login()` now expects English/LTR); new `forceArabicLocale()` fixture helper keeps RTL rendering covered via 2 tests that explicitly force it | **Merged** 2026-08-13 (`f4fa203`); post-merge CI confirmed the RTL regression gone — 6 remaining failures, all `notifications.spec.ts` (PR #50's fix), run `31738895281` |
| **#50** | `fix/e2e-notifications-friday-slot-bug` | Fix: pre-existing Friday/Saturday working-hours gap made `notifications.spec.ts` fail 6 tests | `findAvailableSlot()` always books "tomorrow," but `DemoDataSeeder` only seeds Sunday–Thursday working hours; applied the same fix `appointments.spec.ts`'s `ensureWorkingHours` already uses — unconditionally seed a wide working-hours shift for the target date's actual day-of-week before querying availability. No production code touched | **Merged** 2026-08-13 (`eabd267`); post-merge CI on `main` **fully green — Backend/Frontend/E2E all `success`, 62/62 E2E passed, 0 failures** (run `31748627267`), closing out all 3 regression classes from PR #48 |
...
## 7. AI Assistant (Removed 2026-08-14)

**Status: Removed.** The AI Assistant module (Dashboard Insights, Smart Search, Writing Reports,
Clinical Notes draft-assist, Treatment Suggestions, and the Settings-managed Anthropic API key) was
built via PR #9 (merged 2026-07-31) and extended via PR #11 (merged 2026-08-02), then **fully removed
2026-08-14** per explicit user request following a read-only discovery audit — no replacement AI
capability was added. Full removal account: `CHANGELOG.md`'s "Removed — AI Assistant module" entry.
This section is intentionally kept as a placeholder (not renumbered away) so every other section's
existing `§N` cross-references throughout this file stay valid.

Note: the separate, never-built "Future AI Integration Points" sections referenced from several
`docs/modules/*-design.md` files describe a distinct, speculative future vision — unrelated to this
removed feature and deliberately left untouched.
...
RESOLVED notes on both entries). A 2026-08-18 production-hardening pass closed five more former entries in
one session (see that section's own newest paragraph below for the full account) — **RESOLVED, removed from
this table**: no permanent E2E suite for Billing/Payments/Treatment Plans; Billing's missing backend
Feature-test suite and final doc; the backend suite never exercising real PostgreSQL; orphaned
`ai_interaction_log` i18n/permission-row leftovers. The `App\Rules\BelongsToPatient`/Laboratory SQL-error
item below was found, during that same pass, to have **already been fixed 2026-08-08** (PR #24,
`patient-laboratory-redesign-design.md` §5) — this table's own copy of it had simply never been updated
after the fact; re-verified 2026-08-18 (`LabCaseTest` 33/33 against real PostgreSQL) and removed here too.
Nothing has replaced them as High priority.
...
| No UI links an appointment to a Treatment Plan item, despite full backend support | **Low-Medium** — found 2026-08-18 writing `treatment-plans.spec.ts`; backend (`PUT /treatment-plan-items/{id}` accepts `appointment_id`) and the read-side rendering both already work | Small, additive frontend-only fix; revisit whenever this module's UI is next substantively touched |
...
| Axis | Assessment | % |
|---|---|---|
| **Backend** | 17/17 roadmap modules implemented (16 after AI Assistant's removal), layered consistently (Migration → Model → Service → Policy → Controller), Larastan level 5 clean on every CI run, and — as of the 2026-08-18 pass — verified against real PostgreSQL 17 in CI, not just SQLite. Weakest points: `dental_conditions` pricing catalog is a known-temporary shape; no dentist-to-appointment UI link on Treatment Plan items. | **88%** |
...
| **Testing** | Backend and frontend unit/feature suites are extensive and consistently green; CI's E2E job is fully green; the backend suite now also runs against real PostgreSQL in CI. Treatment Plans, Payments, and Billing — the last modules-with-a-known-E2E-gap — each gained a permanent, CI-verified E2E suite 2026-08-18. | **88%** |
| **Documentation** | Unusually thorough (a design doc + final doc pattern, a dedicated decisions log, a dedicated tech-debt log); the 3 staleness gaps §0 found (2026-08-04) were reconciled 2026-08-07, and this file's §15 protocol + `CLAUDE.md` now exist specifically to keep it from recurring. The 2026-08-18 pass found and fixed two more stale entries this same protocol had missed (`PROJECT_CONTEXT.md`'s frozen AI Assistant paragraph, this table's own already-fixed Laboratory SQL-bug row) — see this file's own newest entries. | **87%** |
| **Security** | Sanctum SPA auth, rate limiting (login + general API), audit logging on sensitive models, policy-gated authorization checked at API + frontend router layers. A real Postgres-transaction bug in `AuditLogService` (silently defeating its own documented fail-open guarantee inside a wrapped transaction) was found and fixed 2026-08-18. Gaps: no malware/virus scanning on uploaded images, no S3/offsite backup yet. | **82%** |
...
| **Maintainability** | PSR-12/Pint/ESLint/Prettier all CI-enforced; `TECH_DEBT.md` and `docs/decisions.md` actively prevent debt from being silently forgotten. Some documentation-sync discipline has slipped recently (§0) — the direct motivation for this file. | **82%** |
| **Production Readiness** | System-Wide Production Gate closed 2026-07-20 (rate limiting, hardened Docker/nginx/SSL, rehearsed backup/restore, CI/CD). `main`'s CI is fully green. 14 of the table's 17 rows now carry the "Production Ready ✅" badge (Treatment Plans/Billing/Payments closed the last gap 2026-08-18) — the remaining 3 (Authentication, Users, Patients) are foundational modules tracked as "Done" rather than against the E2E-suite bar, not modules with a known gap. Remaining gap before onboarding a real clinic: no offsite backup. | **80%** |
...
|---|---|---|---|---|---|
| 1 | **Continue Premium Visual Redesign**: remaining icon migration (~62 files), Phase 3 Data Tables | The active frontend-only initiative — Dashboard redesign (former overlap with roadmap Phase 3) resolved 2026-08-09, see "Resolved this pass" below | Medium (UX quality, not correctness) | Large (multi-step, own design doc already scopes it) | None blocking, but should follow the two-phase design→implementation workflow per file/step |
| 2 | **Provision S3/offsite backup** | Local-disk-only backup doesn't survive VPS loss | Medium, escalates to High before real clinic onboarding | Small (config-only — code path already exists, commented out) | Choice of provider (Backblaze B2 / Wasabi / AWS S3) |
| 3 | **App-wide PWA manifest + service worker** | Every module has been built "PWA-ready" but nothing is installable yet; also roadmap Phase 6 | Low–Medium | Medium | None |
| 4 | **`vue-i18n` runtime-only build + message precompilation** | ~84 KB gzip bundle-size win, app-wide | Low | Small–Medium (needs careful 3-locale regression pass) | None |
| 5 | **Codebase-wide PrimeVue `id`→`inputId` accessibility sweep** | Real but narrow a11y gap, fixed piecemeal so far | Low | Small | None |
| 6 | **Add a "Link Appointment" UI control to Treatment Plan items** | Backend already supports it end-to-end; found 2026-08-18 with no UI to reach it | Low-Medium | Small (additive, no backend change) | None |

**Resolved this pass (2026-08-18 production hardening)**: permanent E2E suites written and CI-verified for
Treatment Plans/Payments/Billing (former item #1); Billing's backend Feature-test gap and final doc closed
(former item #2); a real-Postgres CI job added, and it immediately caught and fixed a genuine
`AuditLogService` transaction bug (former item #4); orphaned `ai_interaction_log` i18n keys/permission rows
purged. See this section's own newest paragraph in the header block, `TECH_DEBT.md`, and `CHANGELOG.md` for
full accounts. **Resolved earlier**: the `BelongsToPatient` SQL-error bug in Laboratory's Form Requests was
fixed as part of Phase 2.4 (PR #24, 2026-08-08) — this file's own copy of that item had gone stale and was
only corrected during the 2026-08-18 pass, see `TECH_DEBT.md`'s Resolved section. **Phase 2.6b: Timeline UI**
merged via PR #32, then PR #33 fixed 2 real bugs post-merge CI's E2E job caught in the E2E spec itself — see
§12. This closes out **Phase 2 (Patient Profile Redesign) in full.** **Dashboard redesign** (overlap between
roadmap Phase 3 and the Premium Visual Redesign's own §6 plan) resolved 2026-08-09 via Dashboard 2.0
(PR #35), which absorbed and implemented it in full — see §12.
...
`CHANGELOG.md` and `TECH_DEBT.md` updated in the same pass per Definition of Done. **PR #41 merged to
`main` 2026-08-12** (merge commit `1de1316`); **post-merge CI re-run and fully green — Backend, Frontend,
...
The honest gaps were proportionate to the pace, not a sign of carelessness: three modules (Billing, Payments,
Treatment Plans) shipped ahead of the E2E-suite convention solidifying and went unbackfilled for three
weeks — closed 2026-08-18 in a dedicated production-hardening pass (see this file's own newest header-cell
entry), which also caught the Laboratory `BelongsToPatient` bug's own fix (2026-08-08) never having been
reflected back into this file's §9/§11 tables — a smaller instance of the exact documentation-lag pattern
§0 already named once. The `dental_conditions`-as-pricing-catalog choice remains *known* debt with a named
revisit condition, not a silent gap — exactly the right way to carry debt, but still debt.
...
row in §4) for almost everything; §5 for architecture/DB decisions; §6 for UI changes; §9

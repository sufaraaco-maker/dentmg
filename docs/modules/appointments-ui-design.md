...
| 6 | `DentistScheduleView.vue` (Working Hours + Time Off, §5-§6) — `meta.roles: ['admin', 'dentist']` route guard already applied 2026-07-16 (§1.3) | Step 1 | ✅ **Completed (2026-07-17)**, full Vitest coverage (214/214), `vue-tsc`/ESLint/Prettier clean (2 pre-existing `no-explicit-any` errors in the unrelated Patients/Users modules aside). Verified against the real dev stack (Docker/Postgres) — full Working Hours (add/edit-as-delete-then-create/toggle-day-active/copy-to/delete) and Time Off (create/conflict-warning/delete) flows driven end to end as both admin (any dentist) and a dentist role (self-service only, read-only working hours) in English/Arabic × light/dark. Three real bugs found and fixed this pass, none of them workarounds (see CHANGELOG): an upstream PrimeVue `DatePicker` defect that silently discarded any typed (not clicked) 24-hour date/time value, patched via `patch-package` since it also affects the already-shipped `AppointmentDialog` (Step 4); a race condition where an in-progress "Add shift" could be silently stranded as unsaved if an unrelated day's edit forced the shifts list to refresh before the create resolved; and a raw `HH:mm:ss` seconds suffix leaking into the dentist's read-only time display. ✅ **User-approved 2026-07-17.** |
| 7 | `AppointmentTypesView.vue` CRUD (§7) — `meta.roles: ['admin']` route guard already applied 2026-07-16 (§1.3) | Step 1 | ✅ **Completed (2026-07-17)**, full Vitest coverage (12/12 new, 226/226 whole suite), `vue-tsc`/ESLint/Prettier clean (same 2 pre-existing `no-explicit-any` errors in the unrelated Patients/Users modules aside). Verified against the real dev stack (Docker/Postgres) — full Create/Edit/Delete/search/"Show inactive" flows driven end to end in English/Arabic × light/dark. One real bug found and fixed this pass (see CHANGELOG): hex color codes bidi-reordered inside the Arabic RTL table (`#F97316` → `F97316#`), fixed by isolating the hex text's direction. ✅ **User-approved 2026-07-17.** |
| 8 | Dashboard widgets (§8) + `PatientAppointmentsPanel.vue` on `PatientDetailView` | Steps 2-5 | ✅ **Completed (2026-07-18)**, full Vitest coverage (17/17 new — `TodayScheduleWidget`, `UpcomingAppointmentsWidget`, `PatientAppointmentsPanel` — 243/243 whole suite), `vue-tsc`/ESLint/Prettier clean. Both widgets and the panel read from `appointments.ts`'s existing shared range cache directly (no new store/service code) — `TodayScheduleWidget`/`UpcomingAppointmentsWidget` call `fetchRange` for their own date window (a no-op if the Board already loaded it); `PatientAppointmentsPanel` deliberately does **not** pass `patient_id` to the API — that would still mark the fetched range "loaded" in the shared cache while only ever storing one patient's rows in it, silently hiding every other patient's appointments from the Board/Dashboard the next time they render that same range. It fetches the same unfiltered range `TimeOffFormDialog` already does and filters client-side instead, bounded to ±3/6 months (a deliberate, documented scope choice — see TECH_DEBT.md). Verified against the real dev stack (Docker/Postgres) as admin/receptionist/dentist × English/Arabic × light/dark — full create-from-dashboard, create-from-patient-panel (with the dialog showing the prefilled patient immediately, no extra fetch), Check In quick-action, and dentist-role `scope="own"` framing (no Waiting/Late/Check-In, "My Schedule Today" title, hidden New-Appointment button on the patient panel) all driven end to end. One real bug found and fixed this pass (see CHANGELOG): `AppointmentDialog`'s `prefill.patient_id` had no way to show a patient summary without a name/code already known, since `PatientSearchSelect` only shows a summary card for an explicit patient object — the Patient Detail panel's "New Appointment" button was the first real caller of `prefill.patient_id`, and would have opened to a confusing blank-looking Patient tab. Fixed by extending `AppointmentPrefill` with an optional display-only `patient` field. Awaiting user approval before Step 9. |

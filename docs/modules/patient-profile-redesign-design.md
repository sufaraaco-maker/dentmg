...
| | |
|---|---|
| **Status** | **Approved 2026-08-07.** Phase 2.1 (Foundation) merged via PR #18 (2026-08-07). Phase 2.2 (Billing) merged via PR #20 (2026-08-08). Phase 2.3 (Medical History) merged via PR #22 (2026-08-08). Phase 2.4 (Laboratory) implemented 2026-08-08 on `feature/patient-profile-phase2-4-laboratory` — its own drill-down design doc, `docs/modules/patient-laboratory-redesign-design.md`, is the authoritative record for that sub-phase (produced by re-auditing actual current code rather than trusting this doc's own §6.2/§6.3/§8/§17 outline, which turned out incomplete — see that doc's §0). See `docs/PROJECT_STATUS.md` §12 for current merge status. §0's decisions and §9A's Security Architecture Decision are binding on every sub-phase below. |
...
| Laboratory | ✅ implemented (Phase 2.4, 2026-08-08) — full tab, same pattern as every other clinical module, see `docs/modules/patient-laboratory-redesign-design.md` | — |
...
| **2.4 Laboratory integration** ✅ done | `Patient::labCases()`, `forPatient` scope on `LabCase`, new patient-scoped route, `patientLabCases` store, `PatientLabCasesPanel.vue` — plus a required fix (not anticipated by this outline) to a real `BelongsToPatient` SQL bug, see `docs/modules/patient-laboratory-redesign-design.md` | Turned out Low-Medium, not just Low — the bug fix and the read/write-tab product decision (§7 of that doc) were real, unanticipated scope this row's original "Low" rating missed |

...
| **Imaging** | Design approved 2026-07-27, implementation in progress on `feature/imaging` (2026-07-28) — per-patient diagnostic image gallery (photos + X-rays), authenticated/policy-checked storage streaming, non-destructive lightbox viewer, mobile camera capture; see `modules/imaging-design.md` and `TECH_DEBT.md` for current status |
| Reports | Not started — planned after Imaging, once the operational modules are complete enough to report on |

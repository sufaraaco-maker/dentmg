...
| **Reports** | Implemented 2026-07-28 on `feature/reports`, not yet merged to `main` — six reports (Production, Collections, A/R Aging, Appointment Analytics, Treatment Plan Acceptance, New Patients), no new tables (`ReportService` reads existing data live), financial reports admin-only via Gate abilities, CSV export (no PDF/scheduling), `DashboardService.monthly_revenue` wired to real data; 855/855 backend tests (21 Reports-specific) + 650/650 frontend Vitest tests green, `vue-tsc`/ESLint/Prettier clean, production build green; permanent E2E suite (`frontend/e2e/reports.spec.ts`, 2 tests) written but not yet CI-confirmed (see `TECH_DEBT.md`); see `modules/reports-design.md` |
| Settings | Not started — planned after Reports, to consolidate system/clinic config once other modules have stabilized |
| AI Assistant | Not started (optional, assistive only) — planned last, since it depends on data/workflow completeness across the other modules for real value; see `PROJECT_CONTEXT.md` |
